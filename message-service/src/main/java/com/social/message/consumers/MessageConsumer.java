...
public class MessageConsumer {
    ...
    @KafkaListener(topics = "UPDATE_MESSAGE", groupId = "message_service_update_message")
    public void listenUpdate(String payload) {
        MessageReqDTO req;
        try {
            objectMapper.configure(FAIL_ON_UNKNOWN_PROPERTIES, false);
            req = objectMapper.readValue(payload, MessageReqDTO.class);
            messageHistoryService.update(req);
        } catch (JsonProcessingException | ChatServiceException e) {
            kafkaTemplate.send("UPDATE_MESSAGE_FAILED", payload);
            throw new RuntimeException(e);
        }
    }
}
